...
create_db_schema_postgresql() {
    ...
    if [ -z "${ZBX_DB_VERSION}" ]; then
        ...
        zcat /usr/share/doc/zabbix-proxy-postgresql/create.sql.gz | psql --quiet \
                --host "${DB_SERVER_HOST}" --port "${DB_SERVER_PORT}" \
                --username "${DB_SERVER_ZBX_USER}" --dbname "${DB_SERVER_DBNAME}" 1>/dev/null || exit 1

        if [ "${ENABLE_TIMESCALEDB,,}" == "true" ]; then
            cat /usr/share/doc/zabbix-proxy-postgresql/timescaledb.sql | psql --quiet \
                --host ${DB_SERVER_HOST} --port ${DB_SERVER_PORT} \
                --username ${DB_SERVER_ZBX_USER} --dbname ${DB_SERVER_DBNAME} 1>/dev/null || exit 1
        fi
...
update_zbx_config() {
    ...
    if [ -z "${ZBX_HOSTNAME}" ] && [ -n "${ZBX_HOSTNAMEITEM}" ]; then
        ...
        update_config_var $ZBX_CONFIG "Hostname" "${ZBX_HOSTNAME:-"zabbix-proxy-postgres"}"
        update_config_var $ZBX_CONFIG "HostnameItem" "${ZBX_HOSTNAMEITEM}"
    fi
